...
def calc_each_cor(cell_a, cell_b, sigma, lag, dt):
    ...
    for i in range(N1):
        ...
        time_match = np.searchsorted(cell_b[1, :], cell_a[1, i])
        ...
        exponent = -(np.multiply(kernel_b, kernel_b) / sigma)
        ...
        numerator += cell_a[0, i] * np.sum(np.multiply(y_select, Kernel))
...
def calc_cor(cell_array,lag_len=0):
    ...
    for i in range(num_features):
        for j in np.arange(i, num_features):
            dtx = (cell_array[i][1][-1] - cell_array[i][1][0]) / num_features
            dty = (cell_array[j][1][-1] - cell_array[j][1][0]) / num_features
