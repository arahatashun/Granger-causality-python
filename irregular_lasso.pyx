...
def irregular_lasso(cell_list, alpha):
    ...
    cdef np.ndarray[DTYPE_t, ndim=2] Am = np.zeros(
        (N1 - B, P * L))  # explanatory variables
    cdef np.ndarray[DTYPE_t, ndim=2] bm = cell_list[0][0, B:N1 + 1].reshape(
        (N1 - B, 1))
    ...
    for i in range(B, N1):
        ...
        for j in range(P):
            ...
            tSelect = np.broadcast_to(cell_list[j][1, :],
                                      (L, cell_list[j][1, :].size)).T
            ySelect = np.broadcast_to(cell_list[j][0, :],
                                      (L, cell_list[j][0, :].size)).T
            # kernel is used as window function??
            Kernel = np.exp(
                -(np.multiply((tij - tSelect), (tij - tSelect)) / SIG))
